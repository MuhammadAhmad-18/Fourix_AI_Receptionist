...
import { recordStockUsageSchema } from "@/modules/inventory/product.schema";
...
export const POST = withApiHandler(async (req: NextRequest) => {
  ...
  const input = recordStockUsageSchema.parse(await req.json());
  return ok(await productController.recordUsage(ctx, input));
});
